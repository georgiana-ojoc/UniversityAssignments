...
public:
    ...
    FunctionOptimum(bool(*newOptimum)(const double first, const double second), const char* newFunctionName, double(*newFunction)(const double* argument, unsigned char dimensions), double newLeftEnd, double newRightEnd, unsigned char newDimensions)
    {
        ...
        fitness = new double[individuals];
        selection = new double[individuals + 1];

        fitnessAverage = new double[generations];
        fitnessMaximum = new double[generations];
    ...
    ~FunctionOptimum()
    {
        ...
        delete[] fitnessAverage;
        delete[] fitnessMaximum;
    ...
    void SinglePoint(default_random_engine generator, unsigned short first, unsigned short second)
    {
    ...
    void MultiplePoints(default_random_engine generator, unsigned short first, unsigned short second)
    {
        if (HammingDistance(first, second) > 10)
        {
            ...
            {
                points.emplace_back(pointDistribution(generator));
            }
    ...
    void SinglePointCrossover()
    {
        ...
        {
            SinglePoint(generator, (individual - 1)->second, individual->second);
        }
        ...
        else
        {
            if (individual == probabilities.end())
            {
                SinglePoint(generator, (individual - 2)->second, (individual - 1)->second);
            }
        }
    }
    ...
    void MultiplePointsCrossover()
    {
        ...
        for (; individual < probabilities.end() && individual->first < crossover; individual += 2)
        {
            MultiplePoints(generator, (individual - 1)->second, individual->second);
        }
        ...
        else
        {
            if (individual == probabilities.end())
            {
                MultiplePoints(generator, (individual - 2)->second, (individual - 1)->second);
            }
        }
    }
    ...
    void CalculateFitness(unsigned short generation)
    {
        ...
        else
        {
            ...
            for (unsigned short iterator = 0; iterator < individuals; ++iterator)
            {
                fitness[iterator] = generationValues[iterator] - 0.99 * minimumValue;
    ...
    unsigned short RouletteWheel()
    {
        ...
        const unsigned int seed = system_clock::now().time_since_epoch().count();
        default_random_engine generator(seed);
        const double position = distribution(generator);
    ...
    void Select(unsigned short generation)
    {
        ...
        for (unsigned short iterator = 0; iterator < individuals; ++iterator)
        {
            memcpy(newPopulation + unsigned long long(chromosomeSize) * iterator, population + unsigned long long(chromosomeSize) * Tournament(), chromosomeSize * sizeof(unsigned char));
        }
        for (unsigned short iterator = 0; iterator < individuals; ++iterator)
        {
            memcpy(population + unsigned long long(chromosomeSize) * iterator, newPopulation + unsigned long long(chromosomeSize) * Tournament(), chromosomeSize * sizeof(unsigned char));
        }
    }
    ...
    void GA()
    {
        ...
        for (unsigned short iterator = 0; iterator < generations; ++iterator)
        {
            Select(iterator);
            double ratio = fitnessAverage[iterator] / fitnessMaximum[iterator];
            printf("%f, %f, %f \n", fitnessAverage[iterator], fitnessMaximum[iterator], ratio);
            if (ratio > 0.9)
            {
...
void FirstFunction(unsigned char time)
{
...
void SecondFunction(unsigned char time)
{
    this_thread::sleep_for(seconds(time));
...
void ThirdFunction(unsigned char time)
{
    this_thread::sleep_for(seconds(time));
...
void FourthFunction(unsigned char time)
{
    this_thread::sleep_for(seconds(time));
...
int main()
{
    ...
    thread firstThread(FirstFunction, 0);
    thread secondThread(SecondFunction, 1);
    thread thirdThread(ThirdFunction, 2);
    thread fourthThread(FourthFunction, 3);
